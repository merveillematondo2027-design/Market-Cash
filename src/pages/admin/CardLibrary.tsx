...
const ELIGIBLE_LOCAL_CARD_ROLES=new Set(['client','agent','marchand','developer','api_partner','creator','agent_administratif','admin_general','chef_agence','designer_graphique','livreur']);
const BLOCKED_ACCOUNT_STATUSES=new Set(['blocked','suspended','banned','deleted']);
...
export default function CardLibrary(){
 ...
 useEffect(()=>{
  let readyCards=false,readyUsers=false,readyLocal=false;
  const done=()=>{if(readyCards&&readyUsers&&readyLocal)setLoading(false)};
  ...
  const unsubLocal=onSnapshot(collection(db,'local_cards'),snap=>{setLocalCards(snap.docs.map(d=>{const raw:any=d.data();return{...raw,id:d.id,cardId:raw.cardId||d.id,cardIdentifier:raw.cardIdentifier||`LOCAL-${d.id.slice(-8).toUpperCase()}`,network:'market_cash',type:'local',status:raw.status||'active',createdAt:Number(raw.createdAt||0),updatedAt:Number(raw.updatedAt||raw.createdAt||0),source:'local_cards',program:raw.program||'market_cash_local',currency:raw.currency||''}as LibraryCard}));readyLocal=true;done()},err=>{console.error('[CARD_LIBRARY_LOCAL]',err);readyLocal=true;done()});
  return()=>{unsubUsers();unsubCards();unsubLocal()};
 },[]);

 const eligibleLocalUsers=useMemo(()=>Object.values(users).filter(user=>ELIGIBLE_LOCAL_CARD_ROLES.has(clean(user.role)||'client')&&!BLOCKED_ACCOUNT_STATUSES.has(clean((user as any).accountStatus)||'active')),[users]);
 const expectedLocalCards=eligibleLocalUsers.length*2;
 ...
 useEffect(()=>{
  if(loading||autoSyncAttempted.current||eligibleLocalUsers.length===0)return;
  if(localCards.length>=expectedLocalCards)return;
  autoSyncAttempted.current=true;
  void syncLocalCards(true);
 },[loading,eligibleLocalUsers.length,expectedLocalCards,localCards.length]);
