...
import{CreditCard,Download,Eye,Filter,Layers,Printer,Search,X}from'lucide-react';

type LibraryCard=UserCard&{source?:'cards'|'local_cards';clientPhone?:string;clientName?:string;clientEmail?:string;program?:string};
...
export default function CardLibrary(){
 ...
 useEffect(()=>{
  ...
  const unsubLocal=onSnapshot(collection(db,'local_cards'),snap=>{setLocalCards(snap.docs.map(d=>{const raw:any=d.data();return{...raw,id:d.id,cardId:raw.cardId||d.id,cardIdentifier:raw.cardIdentifier||`LOCAL-${d.id.slice(-8).toUpperCase()}`,network:'other',type:raw.type||'virtual',status:raw.status||'active',createdAt:Number(raw.createdAt||0),updatedAt:Number(raw.updatedAt||raw.createdAt||0),source:'local_cards',program:raw.program||'market_cash_local'}as LibraryCard}));},err=>console.warn('[CARD_LIBRARY_LOCAL]',err));
 ...
 const filtered=useMemo(()=>allCards.filter(card=>{
  ...
  return[card.cardIdentifier,card.cardId,card.cardNumber,card.cardHolder,card.clientName,card.clientEmail,card.clientPhone,card.userId,card.network,card.type,card.status,card.agencyName,card.agencyId].some(v=>clean(v).toLowerCase().includes(q));
 ...
 return <div className="mx-auto max-w-7xl space-y-6 pb-20">
  <header className="rounded-[2.4rem] bg-blue-950 p-7 text-white shadow-xl"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-400/20 text-amber-400"><Layers/></div><div><h1 className="text-2xl font-black uppercase sm:text-3xl">Bibliothèque des cartes</h1><p className="text-xs font-semibold text-blue-200">Toutes les cartes utilisateurs • recherche • impression PVC</p></div></div></header>
