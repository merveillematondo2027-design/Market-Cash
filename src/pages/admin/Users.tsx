...
import{AlertTriangle,BadgeCheck,Ban,CreditCard,Edit3,FileText,Filter,Fingerprint,History,KeyRound,LockKeyhole,NotebookPen,RefreshCw,RotateCcw,Search,Shield,ShieldCheck,Snowflake,Trash2,Truck,User as UserIcon,WalletCards,X}from'lucide-react';
...
const roleLabel=(role:UserRole)=>({client:'Client',agent:'Agent point de vente',marchand:'Marchand / Business',developer:'Développeur',api_partner:'Partenaire API',agent_administratif:'Agent administratif',chef_agence:"Chef d'agence",designer_graphique:'Designer graphique',livreur:'Livreur',admin_general:'Admin général'}[role]||role);
const effectiveRole=(user:User):UserRole=>{
  if(user.role==='api_partner'||user.businessAccountType==='api_provider'||user.apiProviderEnabled===true)return'api_partner';
  if(user.role==='developer'||user.businessAccountType==='direct_developer'||(user.developerEnabled===true&&user.role==='marchand'))return'developer';
  return user.role;
};
...
export default function AdminUsers(){
  ...
  async function openUser(target:User,updateUrl=true){
    ...
    logService.audit('CLIENT_PROFILE_VIEWED','Dossier utilisateur consulté',{targetType:'user',targetId:target.uid,targetUserId:target.uid,targetRole:effectiveRole(target)});
    try{
  ...
  function editRole(target:User){
    ...
    setEditingUser(target);setSelectedRole(effectiveRole(target));setAgencyName(target.agencyName||target.agencyId||'');
  }
  ...
  async function saveRole(e:React.FormEvent){
    ...
    const targetUid=editingUser.uid;const previousRole=effectiveRole(editingUser);
    setIsSaving(true);
    try{
      await adminUserService.setRole(targetUid,selectedRole,agencyName.trim());
      logService.audit('USER_ROLE_CHANGED','Rôle utilisateur modifié',{targetType:'user',targetId:targetUid,targetUserId:targetUid,oldRole:previousRole,newRole:selectedRole,success:true});
      toast.success('Rôle et accès mis à jour.');setEditingUser(null);await refreshSelected(targetUid);
    }catch(error:any){toast.error(error?.message||firestoreErrorMessage(error,'Modification du rôle impossible.'))}
  ...
  async function saveIdentity(){
    ...
    setIsSaving(true);
    try{
      await adminUserService.setIdentity(selectedUser.uid,profileName.trim(),profilePhone.trim());
    ...
    }catch(error:any){toast.error(error?.message||firestoreErrorMessage(error,'Impossible de modifier le profil.'))}
    finally{setIsSaving(false)}
  }
  ...
  async function refreshSelected(uid:string){
    ...
    if(snap.exists()){
      const fresh={...snap.data(),uid:snap.id}as User;
      setSelectedUser(fresh);setProfileName(fresh.displayName||'');setProfilePhone(fresh.phone||'');
    }
    await Promise.all([loadUsers(),loadControl(uid)]);
  }
  ...
  async function accountAction(status:'active'|'suspended'|'blocked'){
    ...
    if(status==='suspended'){
      const value=window.prompt('Durée de suspension en heures (ex. 1, 24, 72, 168) :','24');if(value===null)return;
      const hours=Number(value.replace(',','.'));if(!Number.isFinite(hours)||hours<=0||hours>8760)return toast.error('Durée invalide. Utilisez un nombre d’heures entre 0 et 8760.');durationMinutes=Math.round(hours*60);
    }
    ...
    try{const result=await adminUserService.setAccountStatus(selectedUser.uid,status,durationMinutes);const suspendedUntil=result.suspendedUntil||0;toast.success(status==='active'?'Compte réactivé.':status==='suspended'?`Compte suspendu jusqu’au ${fmt(suspendedUntil)}.`:'Compte bloqué et wallets gelés.');await refreshSelected(selectedUser.uid)}
  ...
  async function deleteAccount(){
    ...
    const token=window.prompt(`SUPPRESSION DU COMPTE\n\nL’utilisateur pourra recréer un nouveau compte plus tard avec la même adresse e-mail.\nTapez SUPPRIMER pour confirmer :`,'');if(token!=='SUPPRIMER')return;
    setActionLoading('delete-account');
    try{await adminUserService.deleteAccount(selectedUser.uid);toast.success('Compte supprimé. Cette adresse e-mail pourra se réinscrire plus tard.');await refreshSelected(selectedUser.uid)}
  ...
  async function banAccount(){
    ...
    const reason=window.prompt(`BANNISSEMENT DÉFINITIF\n\nL’adresse ${selectedUser.email} ne pourra plus créer ni utiliser un compte Market-Cash.\nIndiquez le motif :`,'Fraude / violation des conditions Market-Cash')?.trim();if(!reason)return;
    const token=window.prompt('Tapez BANNIR pour confirmer définitivement :','');if(token!=='BANNIR')return;
    setActionLoading('ban-account');
    try{await adminUserService.banAccount(selectedUser.uid,reason);toast.success('Compte et adresse e-mail bannis définitivement.');await refreshSelected(selectedUser.uid)}
  ...
  async function walletAction(currency:'USD'|'CDF'|'ALL',status:'active'|'frozen'){
    if(!selectedUser||actionLoading)return;setActionLoading(`wallet:${currency}:${status}`);
  ...
  async function resetPin(){
    if(!selectedUser||actionLoading)return;if(!window.confirm('Réinitialiser le PIN de cet utilisateur ? Le PIN actuel sera supprimé et la biométrie désactivée.'))return;setActionLoading('reset-pin');
    try{await adminUserService.resetPin(selectedUser.uid);toast.success('PIN réinitialisé. Le client devra en créer un nouveau.');await refreshSelected(selectedUser.uid)}
  ...
  async function disableBiometrics(){
    ...
    try{await adminUserService.disableBiometrics(selectedUser.uid);toast.success('Biométrie désactivée.');await refreshSelected(selectedUser.uid)}
    catch(error:any){toast.error(error?.message||'Action impossible.')}
    finally{setActionLoading('')}
  }
  ...
  const filtered=useMemo(()=>{
    ...
    const result=users.filter(u=>{const role=effectiveRole(u);const q=searchQuery.trim().toLowerCase();const search=!q||[u.displayName,u.email,u.phone,role,roleLabel(role),u.agencyName,u.agencyId,u.accountStatus].some(v=>String(v||'').toLowerCase().includes(q));const complete=Boolean(u.displayName?.trim()&&u.email?.trim()&&u.phone?.trim());const created=Number(u.createdAt||0);return search&&(roleFilter==='all'||role===roleFilter)&&(agencyFilter==='all'||(u.agencyName||u.agencyId||'')===agencyFilter)&&(profileFilter==='all'||(profileFilter==='complete'&&complete)||(profileFilter==='incomplete'&&!complete)||(profileFilter==='with_phone'&&!!u.phone?.trim())||(profileFilter==='without_phone'&&!u.phone?.trim()))&&(periodFilter==='all'||(periodFilter==='today'&&created>=now-day)||(periodFilter==='7d'&&created>=now-7*day)||(periodFilter==='30d'&&created>=now-30*day))});
  ...
  const countRole=(role:UserRole)=>users.filter(u=>effectiveRole(u)===role).length;
  const developerCount=countRole('developer')+countRole('api_partner');
  ...
  return <div className="mx-auto max-w-7xl space-y-4 pb-24 px-1 sm:px-0">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-xl font-black text-blue-950 sm:text-2xl">Clients & Wallets</h1><p className="text-xs text-slate-500">Gestion complète des comptes, rôles, accès, sécurité, wallets et activités.</p></div><div className="relative sm:w-80"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Nom, email, téléphone, rôle, statut..." className="w-full rounded-xl border bg-white py-2.5 pl-9 pr-3 text-sm outline-none focus:border-blue-500"/></div></div>
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-6"><Stat label="Utilisateurs" value={users.length}/><Stat label="Clients" value={countRole('client')}/><Stat label="Agents" value={countRole('agent')}/><Stat label="Marchands" value={countRole('marchand')}/><Stat label="Developers" value={developerCount}/><Stat label="Personnel" value={users.filter(u=>!['client','agent','marchand','developer','api_partner'].includes(effectiveRole(u))).length}/></div>
    ...
    <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-blue-950"><Filter size={16}/><span className="text-sm font-black">Filtrer & trier</span>{activeFilterCount>0&&<span className="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-black text-blue-700">{activeFilterCount}</span>}</div><button onClick={resetFilters} className="flex items-center gap-1 text-[11px] font-black text-slate-500"><RotateCcw size={13}/>Réinitialiser</button></div>
      <div className="flex gap-2 overflow-x-auto pb-1">{[['all','Tous'],['client','Clients'],['agent','Agents'],['marchand','Marchands'],['developer','Développeurs'],['api_partner','Partenaires API'],['admin_general','Admins'],['chef_agence','Chefs agence'],['agent_administratif','Agents admin'],['designer_graphique','Designers'],['livreur','Livreurs']].map(([value,label])=><button key={value} onClick={()=>setRoleFilter(value as RoleFilter)} className={`shrink-0 rounded-xl border px-3 py-2 text-[11px] font-black ${roleFilter===value?'border-blue-950 bg-blue-950 text-white':'border-slate-200 bg-slate-50 text-slate-600'}`}>{label}</button>)}</div>
    ...
    {selectedUser&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-2 sm:p-5" onMouseDown={e=>{if(e.target===e.currentTarget)closeUser()}}><div className="flex max-h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-slate-50 shadow-2xl">
      <div className="flex justify-between gap-3 bg-blue-950 p-4 text-white"><div className="flex min-w-0 gap-3"><Avatar user={selectedUser} large/><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-lg font-black">{selectedUser.displayName||'Utilisateur Market-Cash'}</h2><StatusBadge status={(control?.accountStatus||selectedUser.accountStatus)as AccountStatus}/><RoleBadge role={effectiveRole(selectedUser)}/></div><div className="truncate text-xs text-blue-200">{selectedUser.email}</div><div className="mt-1 truncate font-mono text-[9px] text-blue-300">UID {selectedUser.uid}</div></div></div><button onClick={closeUser} className="shrink-0 rounded-full bg-white/10 p-2"><X size={18}/></button></div>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-5">{detailLoading?<div className="p-12 text-center font-bold text-slate-500">Chargement du dossier...</div>:<>
        {tab==='control'&&<div className="space-y-4">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-5"><Mini label="Cartes" value={cards.length}/><Mini label="Demandes" value={requests.length}/><Mini label="Livraisons" value={deliveries.length}/><Mini label="KYC" text={selectedUser.kycStatus||'not_started'}/><Mini label="Rôle" text={roleLabel(effectiveRole(selectedUser))}/></div>
          <section className="rounded-2xl border bg-white p-4"><div className="mb-3 flex items-center gap-2"><Edit3 size={17} className="text-blue-800"/><h3 className="font-black text-slate-900">Informations du compte</h3></div><div className="grid gap-3 sm:grid-cols-2"><Field label="Nom complet"><input value={profileName} onChange={e=>setProfileName(e.target.value)} className="w-full rounded-xl border p-3 text-sm"/></Field><Field label="Téléphone"><input value={profilePhone} onChange={e=>setProfilePhone(e.target.value)} className="w-full rounded-xl border p-3 text-sm"/></Field><Info label="Email" value={selectedUser.email}/><Info label="Inscription" value={fmt(selectedUser.createdAt)}/><Info label="Agence" value={selectedUser.agencyName||selectedUser.agencyId}/><Info label="Dernière mise à jour" value={fmt(selectedUser.updatedAt)}/></div>{currentUser?.role==='admin_general'&&<div className="mt-4 flex flex-wrap gap-2"><button disabled={isSaving} onClick={saveIdentity} className="rounded-xl bg-blue-950 px-4 py-2.5 text-xs font-black text-white">Enregistrer identité</button><button disabled={isSaving} onClick={()=>editRole(selectedUser)} className="rounded-xl border border-blue-950 px-4 py-2.5 text-xs font-black text-blue-950">Rôle & accès</button></div>}</section>
          <section className="rounded-2xl border bg-white p-4"><div className="mb-3 flex items-center gap-2"><ShieldCheck size={18} className="text-emerald-600"/><div><h3 className="font-black">État & conformité</h3><p className="text-xs text-slate-500">Les décisions KYC et professionnelles restent traitées dans le module KYC & Comptes.</p></div></div><div className="grid gap-2 sm:grid-cols-4"><ControlInfo label="Compte" value={statusLabel(control?.accountStatus||selectedUser.accountStatus)}/><ControlInfo label="KYC" value={selectedUser.kycStatus||'not_started'}/><ControlInfo label="Accès" value={roleLabel(effectiveRole(selectedUser))}/><ControlInfo label="Suspension jusqu’au" value={control?.suspendedUntil?fmt(control.suspendedUntil):'—'}/></div><div className="mt-3 flex flex-wrap gap-2"><button onClick={()=>navigate('/admin/account-requests')} className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700">Ouvrir KYC & Comptes</button>{currentUser?.role==='admin_general'&&<><button disabled={!!actionLoading} onClick={()=>accountAction('active')} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">Réactiver</button><button disabled={!!actionLoading} onClick={()=>accountAction('suspended')} className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">Suspendre avec durée</button><button disabled={!!actionLoading} onClick={()=>accountAction('blocked')} className="rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-red-700">Bloquer</button></>}</div></section>
