...
export default function CardsHub(){
  ...
  useEffect(()=>{
    ...
    },error=>{console.warn('[CARDS_HUB_VISA_LIST_ERROR]',error);setVisaCards([])});
  },[user?.uid,visaMode]);
  ...
  const askReveal=(kind:CardProductVariant,id:string)=>{
    if(revealed[id]){setRevealed(current=>({...current,[id]:false}));if(kind==='local')setLocalSecure(null);return}
  ...
  return <div className="mx-auto max-w-4xl px-3.5 pb-28 pt-4 sm:px-6">
    ...
    <section className="mb-8"><div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-black uppercase tracking-[.12em] text-blue-950">Market-Cash Locale</h2>{loadingLocal&&<RefreshCw size={15} className="animate-spin text-slate-400"/>}</div>
      <Link to={localCard?`/client/cards?card=local&cardId=${encodeURIComponent(localCard.cardId)}`:'/client/cards?card=local'} className="block">
        <CardProductFace variant="local" holder={localSecure?.cardHolder||localCard?.cardHolder||user?.displayName} number={localSecure?.cardNumber||localCard?.maskedNumber} expiryStart={localSecure?.expiryStart} expiryEnd={localSecure?.expiryEnd} cvv={localSecure?.cvv} revealed={!!revealed[localKey]} onToggleReveal={()=>askReveal('local',localKey)} className="transition duration-200 active:scale-[.985]"/>
      </Link>
    </section>

    <section className="mb-8"><div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-black uppercase tracking-[.12em] text-slate-700">Market-Cash Visa Standard</h2>{standardCards.length<4&&<Link to="/client/cards?card=standard" className="grid h-8 w-8 place-items-center rounded-full bg-slate-100 text-slate-700" aria-label="Ajouter une Visa Standard"><Plus size={16}/></Link>}</div>
      {standardCards.length>0?<div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{standardCards.map(card=>{const id=card.cardId||card.id||'';return <Link key={id} to={`/client/cards?card=standard&cardId=${encodeURIComponent(id)}`} className="w-[88%] shrink-0 snap-center sm:w-[68%] md:w-[58%]"><CardProductFace variant="standard" holder={card.cardHolder||card.cardHolderName||user?.displayName} number={card.cardNumber} expiryStart={card.expiryStart||card.validFrom} expiryEnd={card.expiryEnd||card.expiry||card.validUntil} cvv={card.cvv} revealed={!!revealed[id]} onToggleReveal={()=>askReveal('standard',id)} className="transition duration-200 active:scale-[.985]"/></Link>})}</div>:<Link to="/client/cards?card=standard" className="block"><CardProductFace variant="standard" holder={user?.displayName} revealed={false} onToggleReveal={()=>toast('Aucune Visa Standard attribuée.',{icon:'💳'})} className="transition duration-200 active:scale-[.985]"/></Link>}
    </section>

    <section><div className="mb-3 flex items-center justify-between"><h2 className="text-sm font-black uppercase tracking-[.12em] text-amber-800">Market-Cash Visa Gold</h2></div>
      <Link to={goldCard?`/client/cards?card=gold&cardId=${encodeURIComponent(goldCard.cardId||goldCard.id||'')}`:'/client/cards?card=gold'} className="block"><CardProductFace variant="gold" holder={goldCard?.cardHolder||goldCard?.cardHolderName||user?.displayName} number={goldCard?.cardNumber} expiryStart={goldCard?.expiryStart||goldCard?.validFrom} expiryEnd={goldCard?.expiryEnd||goldCard?.expiry||goldCard?.validUntil} cvv={goldCard?.cvv} revealed={goldCard?!!revealed[goldCard.cardId||goldCard.id||'']:false} onToggleReveal={()=>goldCard?askReveal('gold',goldCard.cardId||goldCard.id||''):toast('Visa Gold disponible après émission partenaire.',{icon:'✨'})} className="transition duration-200 active:scale-[.985]"/></Link>
    </section>
