  ...
  return <div className="mx-auto max-w-xl p-4 pb-28 md:p-8">
    ...
    <section className="mt-5 rounded-[2rem] border bg-white p-5 shadow-sm md:p-6">
      ...
      {loadingCard?<div className="mt-5 rounded-2xl bg-slate-50 p-4 text-center text-sm font-bold text-slate-500">Chargement de la carte locale…</div>:!card?<div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Aucune carte locale disponible.</b><Link to="/client/cards" className="mt-3 block rounded-xl bg-blue-950 px-4 py-3 text-center font-black text-white">Ouvrir Mes cartes</Link></div>:<>
        <div className="mt-5 rounded-2xl bg-blue-50 p-4 text-blue-950"><div className="flex items-center gap-2"><CreditCard size={17}/><span className="text-xs font-black uppercase tracking-wide">Carte locale utilisée</span></div><div className="mt-3 flex items-center justify-between gap-3"><div><p className="font-mono text-sm font-black">{card.maskedNumber}</p><p className="mt-1 text-xs font-bold text-slate-500">Une seule carte locale par client</p></div><div className="text-right"><p className="text-[10px] font-black uppercase text-slate-400">Solde {currency}</p><p className="font-black">{fmt(cardBalance,currency)}</p></div></div></div>
