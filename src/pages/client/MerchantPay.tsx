...
export default function MerchantPay(){
  ...
  return <div className="mx-auto max-w-xl p-4 pb-28 md:p-8">
    ...
    <section className="mt-5 rounded-[2rem] border bg-white p-5 shadow-sm md:p-6">
      ...
      <p className="mt-1 text-sm leading-6 text-slate-500">Le paiement est débité de votre unique carte locale Market-Cash. La confirmation financière se fait avec son CVV à 3 chiffres.</p>
      {initialMerchant&&<div className="mt-3 rounded-2xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800">QR / lien marchand détecté. Vérifiez le montant et le bénéficiaire.</div>}
      ...
      {loadingCard?<div className="mt-5 rounded-2xl bg-slate-50 p-4 text-center text-sm font-bold text-slate-500">Chargement de la carte locale…</div>:!card?<div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Aucune carte locale disponible.</b><Link to="/client/cards" className="mt-3 block rounded-xl bg-blue-950 px-4 py-3 text-center font-black text-white">Ouvrir Mes cartes</Link></div>:<>
        <div className="mt-5 rounded-2xl bg-blue-50 p-4 text-blue-950"><div className="flex items-center gap-2"><CreditCard size={17}/><span className="text-xs font-black uppercase tracking-wide">Carte locale utilisée</span></div><div className="mt-3 flex items-center justify-between gap-3"><div><p className="font-mono text-sm font-black">{card.maskedNumber}</p><p className="mt-1 text-xs font-bold text-slate-500">Une seule carte locale par client</p></div><div className="flex items-center gap-2"><div className="text-right"><p className="text-[10px] font-black uppercase text-slate-400">Solde {currency}</p><p className="font-black">{sensitive.revealed?fmt(cardBalance,currency):'••••••'}</p></div><button type="button" onClick={sensitive.request} className="grid h-10 w-10 place-items-center rounded-xl bg-white text-blue-950 shadow-sm" aria-label={sensitive.revealed?'Masquer le solde':'Afficher le solde'}>{sensitive.revealed?<EyeOff size={18}/>:<Eye size={18}/>}</button></div></div></div>

        {step==='form'&&<div className="mt-5 space-y-3"><label className="block text-xs font-black uppercase text-slate-500">ID Market-Cash du marchand</label><input value={merchantId} onChange={e=>setMerchantId(e.target.value.toUpperCase())} placeholder="MCW-XXXXXXXXXX" className="w-full rounded-2xl border p-4 font-mono"/><label className="block text-xs font-black uppercase text-slate-500">Montant</label><input value={amount} onChange={e=>setAmount(e.target.value)} inputMode="decimal" placeholder={`Montant en ${currency}`} className="w-full rounded-2xl border p-4"/>{value>cardBalance&&<p className="text-xs font-bold text-red-600">Solde de la carte locale insuffisant.</p>}<button disabled={busy||!merchantId.trim()||!valid} onClick={identify} className="w-full rounded-2xl bg-blue-950 py-4 font-black text-white disabled:opacity-40">{busy?'Vérification…':'Vérifier le marchand'}</button></div>}
    ...
    <SecurityConfirmModal open={sensitive.open} busy={sensitive.busy} onClose={sensitive.close} onConfirm={sensitive.confirm} title="Afficher le solde de la carte" subtitle="Entrez le code secret de l’application. Le paiement reste confirmé séparément par le CVV à 3 chiffres."/>
  </div>;
}
