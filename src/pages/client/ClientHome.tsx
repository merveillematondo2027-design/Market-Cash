...
export default function ClientHome(){
  ...
  const actions=[
    ...
    {n:'Retrait',s:'Chez un agent agréé',i:Banknote,to:'/client/wallet/withdraw'},
    {n:'Payer',s:'Un marchand vérifié',i:Store,to:'/client/wallet/pay'},
  ...
  return <div className="mx-auto max-w-5xl space-y-5 p-4 pb-28 md:p-8">
    ...
    <section className="grid gap-3 md:grid-cols-2"><Link to={`/client/wallet?currency=${currency}`} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><WalletCards className="text-blue-800"/><h2 className="mt-3 font-black text-slate-950">Portefeuille principal {currency}</h2><p className="mt-1 text-sm text-slate-500">Il reçoit les dépôts, permet les transferts Market-Cash, les paiements marchands et les retraits autorisés chez un Agent point de vente.</p></Link><Link to="/client/cards" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><CreditCard className="text-blue-800"/><h2 className="mt-3 font-black text-slate-950">Mes cartes</h2><p className="mt-1 text-sm text-slate-500">Les cartes internes sont alimentées uniquement depuis votre portefeuille principal. La Visa virtuelle reste séparée.</p></Link></section>
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-slate-100"><ArrowUpRight className="text-slate-700"/></div><div><h2 className="font-black text-slate-950">Dépôt externe sécurisé</h2><p className="text-sm text-slate-500">Seulement Mobile Money ou banque. Market-Cash transmet la demande au backend, MHT APIs orchestre le partenaire, et le portefeuille n'est crédité qu'après confirmation positive du réseau ou de la banque.</p></div></div></section>
  </div>
}
