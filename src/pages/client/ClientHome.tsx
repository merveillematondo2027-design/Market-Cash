...
export default function ClientHome(){
  ...
  return <div className="mx-auto max-w-5xl space-y-5 p-4 pb-28 md:p-8">
    ...
    <section className="grid gap-3 md:grid-cols-2"><Link to={`/client/wallet?currency=${currency}`} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><WalletCards className="text-blue-800"/><h2 className="mt-3 font-black text-slate-950">Portefeuille principal {currency}</h2><p className="mt-1 text-sm text-slate-500">Il reçoit les dépôts et permet les transferts internes Market-Cash. Pour dépenser, rechargez d’abord votre carte locale.</p></Link><Link to="/client/cards" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><CreditCard className="text-blue-800"/><h2 className="mt-3 font-black text-slate-950">Mes cartes</h2><p className="mt-1 text-sm text-slate-500">Carte locale, Visa Standard et Visa Gold sont gérées séparément dans le module Cartes.</p></Link></section>
    ...
    <SecurityConfirmModal open={securityOpen} busy={securityBusy} onClose={()=>!securityBusy&&setSecurityOpen(false)} onConfirm={confirmReveal} title="Afficher mon solde" subtitle="Entrez le code secret de l’application pour afficher le solde et votre identifiant Market-Cash. Le CVV reste réservé à la confirmation des transactions."/>
  </div>;
}
