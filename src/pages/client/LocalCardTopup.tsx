...
import{ArrowLeft,CheckCircle2,CreditCard,Eye,EyeOff,WalletCards}from'lucide-react';
...
export default function LocalCardTopup(){
  ...
  const submit=async()=>{
    ...
    setBusy(true);
    try{const result=await agentWalletService.fundInternalCard({cardId:card.cardId,currency,amount:numericAmount,cvv,idempotencyKey:makeKey()});setReference(result.reference);setDone(true);setAmount('');setCvv('');await load();toast.success('Carte locale rechargée.')}catch(error:any){toast.error(error?.message||'Recharge refusée. Vérifiez votre CVV.')}finally{setBusy(false)}
  };
  ...
  return <div className="mx-auto max-w-xl p-4 pb-28 md:p-8">
    ...
    <section className="mt-5 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      <div className="bg-gradient-to-br from-blue-950 to-blue-800 p-6 text-white"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-200">Market-Cash Locale</p><h1 className="mt-2 text-2xl font-black">Recharger ma carte</h1></div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-white/10"><WalletCards size={24}/></div></div><p className="mt-4 text-sm text-blue-100">Wallet principal → votre unique carte locale.</p></div>
      <div className="space-y-5 p-5 md:p-6">
        {loading?<div className="rounded-2xl bg-slate-50 p-5 text-center text-sm font-bold text-slate-500">Chargement…</div>:!card?<div className="rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-900">Aucune carte locale active.</div>:<div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-blue-900"><CreditCard/></div><div className="min-w-0 flex-1"><p className="truncate font-black text-slate-950">{card.cardHolder}</p><p className="font-mono text-sm font-black text-blue-950">{card.maskedNumber}</p></div><button onClick={secure.request} className="grid h-10 w-10 place-items-center rounded-xl bg-white text-blue-950" aria-label="Afficher les soldes">{secure.revealed?<EyeOff size={18}/>:<Eye size={18}/>}</button></div><div className="mt-4 grid grid-cols-2 gap-3 border-t border-blue-100 pt-4"><div><p className="text-[9px] font-black uppercase text-slate-400">Solde wallet</p><p className="mt-1 font-black text-slate-950">{secure.revealed?money(walletBalance,currency):'••••••'}</p></div><div><p className="text-[9px] font-black uppercase text-slate-400">Solde carte</p><p className="mt-1 font-black text-slate-950">{secure.revealed?money(cardBalance,currency):'••••••'}</p></div></div></div>}
        ...
        {done?<div className="rounded-3xl bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto text-emerald-600" size={42}/><h2 className="mt-3 text-xl font-black text-emerald-900">Recharge confirmée</h2><p className="mt-2 text-sm text-emerald-700">Votre carte locale a été créditée.</p><p className="mt-3 font-mono text-xs font-bold text-emerald-800">{reference}</p><button onClick={()=>setDone(false)} className="mt-5 rounded-2xl bg-white px-5 py-3 text-sm font-black text-emerald-800">Nouvelle recharge</button></div>:<>
          ...
          <label className="block"><span className="text-xs font-black uppercase tracking-wide text-slate-500">CVV Market-Cash · 3 chiffres</span><p className="mt-1 text-[11px] leading-5 text-slate-400">Le CVV confirme la transaction. Le code secret de l'application sert uniquement à consulter les informations sensibles.</p><input value={cvv} onChange={e=>setCvv(e.target.value.replace(/\D/g,'').slice(0,3))} type="password" inputMode="numeric" autoComplete="off" placeholder="•••" className="mt-2 w-full rounded-2xl border-2 border-slate-200 px-4 py-4 text-center text-2xl font-black tracking-[.45em] outline-none focus:border-blue-600"/></label>
    ...
    <SecurityConfirmModal open={secure.open} busy={secure.busy} onClose={secure.close} onConfirm={secure.confirm} title="Afficher les soldes" subtitle="Entrez votre code secret de l'application. Il n'est jamais utilisé pour confirmer la recharge."/>
  </div>;
}
