...
export default function LocalCardTopup(){
  ...
  const load=async()=>{const[wallet,cards]=await Promise.all([agentWalletService.getMyWallets(),agentWalletService.getMyInternalCards()]);setServer(wallet);setCard(cards[0]||null)};
  ...
  const submit=async()=>{
    if(!card||!validAmount||code.length!==3)return;
    setBusy(true);
    try{const result=await agentWalletService.fundInternalCard({cardId:card.cardId,currency,amount:numericAmount,cvv:code,idempotencyKey:makeKey()});setReference(result.reference);setDone(true);setAmount('');setCode('');await load();toast.success('Carte locale rechargée.')}catch(error:any){const raw=String(error?.message||'');toast.error(/^internal\s*\[?0\]?$/i.test(raw.trim())?'Le service de recharge est en cours de déploiement. Réessayez dans quelques instants.':raw||'Recharge refusée.')}finally{setBusy(false)}
  };
  ...
  return <div className="mx-auto max-w-xl p-4 pb-28 md:p-8">
    ...
    <section className="mt-5 overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      <div className="bg-gradient-to-br from-blue-950 to-blue-800 p-6 text-white"><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-200">Market-Cash Locale</p><h1 className="mt-2 text-2xl font-black">Recharger ma carte</h1><p className="mt-3 text-sm text-blue-100">Le montant sera transféré depuis votre Wallet vers cette carte.</p></div>
      <div className="space-y-5 p-5 md:p-6">
        {loading?<div className="rounded-2xl bg-slate-50 p-5 text-center text-sm font-bold text-slate-500">Chargement…</div>:!card?<div className="rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-900">Aucune carte locale active.</div>:null}
        <div className="flex items-center justify-between gap-3"><div className="inline-flex rounded-2xl bg-slate-100 p-1">{(['USD','CDF']as WalletCurrency[]).map(item=><button key={item} onClick={()=>{setCurrency(item);setDone(false)}} className={`rounded-xl px-5 py-2 text-xs font-black ${currency===item?'bg-white text-blue-950 shadow-sm':'text-slate-500'}`}>{item}</button>)}</div><p className="text-right text-[11px] font-bold text-slate-400">Disponible<br/><span className="text-sm text-slate-700">{money(walletBalance,currency)}</span></p></div>
        {done?<div className="rounded-3xl bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto text-emerald-600" size={42}/><h2 className="mt-3 text-xl font-black text-emerald-900">Recharge confirmée</h2><p className="mt-2 text-sm text-emerald-700">Votre carte locale a été créditée.</p><p className="mt-3 font-mono text-xs font-bold text-emerald-800">{reference}</p><button onClick={()=>setDone(false)} className="mt-5 rounded-2xl bg-white px-5 py-3 text-sm font-black text-emerald-800">Nouvelle recharge</button></div>:<>
          ...
          <label className="block"><span className="text-xs font-black uppercase tracking-wide text-slate-500">Code de confirmation de la carte · 3 chiffres</span><input value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,'').slice(0,3))} type="password" inputMode="numeric" autoComplete="off" placeholder="•••" className="mt-2 w-full rounded-2xl border-2 border-slate-200 px-4 py-4 text-center text-2xl font-black tracking-[.45em] outline-none focus:border-blue-600"/></label>
          <button disabled={busy||!card||!validAmount||code.length!==3} onClick={submit} className="w-full rounded-2xl bg-blue-950 py-4 font-black text-white shadow-sm disabled:opacity-40">{busy?'Traitement…':`Confirmer la recharge · ${validAmount?money(numericAmount,currency):currency}`}</button>
        </>}
      </div>
    </section>
  </div>;
