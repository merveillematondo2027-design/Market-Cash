...
export default function CardDetail({kind,cardId}:CardDetailProps){
  ...
  return <div className="mx-auto max-w-4xl px-3.5 pb-28 pt-4 sm:px-6">
    ...
    {kind==='local'&&<section className="mx-auto mt-5 grid max-w-xl grid-cols-3 gap-2.5"><Link to={`/client/cards/local/topup?currency=${currency}`} className="rounded-2xl border border-slate-200 bg-white px-2 py-4 text-center shadow-sm"><WalletCards className="mx-auto text-blue-700" size={22}/><span className="mt-2 block text-[11px] font-black text-slate-700">Recharger</span></Link><Link to={`/client/wallet/pay?currency=${currency}`} className="rounded-2xl border border-slate-200 bg-white px-2 py-4 text-center shadow-sm"><Store className="mx-auto text-blue-700" size={22}/><span className="mt-2 block text-[11px] font-black text-slate-700">Payer</span></Link><Link to={`/client/wallet/withdraw?currency=${currency}`} className="rounded-2xl border border-slate-200 bg-white px-2 py-4 text-center shadow-sm"><Banknote className="mx-auto text-blue-700" size={22}/><span className="mt-2 block text-[11px] font-black text-slate-700">Retrait</span></Link></section>}
