...
import { LogOut, Truck, User, Phone } from 'lucide-react';
...
export default function DeliveryProfile() {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">Profil Livreur</h1>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col items-center text-center relative overflow-hidden space-y-4">
        <div className="w-24 h-24 bg-emerald-600 rounded-3xl flex items-center justify-center text-white border-4 border-white shadow-lg">
          <Truck size={40} />
        </div>

        <div>
          <h2 className="text-xl font-black text-slate-800 tracking-tight">
            {user.displayName || 'Livreur Terrain'}
          </h2>
          <p className="text-xs text-slate-500 font-medium">{user.email}</p>
        </div>

        <span className="bg-emerald-100 text-emerald-900 border border-emerald-200 px-3.5 py-1 rounded-xl text-xs font-black uppercase tracking-wider">
          Service Logistique & Livraisons
        </span>
        ...
        <div className="w-full bg-slate-50 rounded-2xl p-4 border border-slate-100 text-left space-y-2 text-xs">
          <div className="flex justify-between py-1 border-b border-slate-200/60">
            ...
            <span className="font-bold text-slate-800">Livreur</span>
          </div>
          <div className="flex justify-between py-1 border-b border-slate-200/60">
            <span className="text-slate-400">Téléphone de contact :</span>
            <span className="font-bold text-slate-800">{user.phone || 'Non renseigné'}</span>
          </div>
