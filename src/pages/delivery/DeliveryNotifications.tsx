...
export default function DeliveryNotifications() {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where('userId', 'in', [user.uid, 'livreur', 'all'])
    );
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      console.error('[DELIVERY_NOTIFICATIONS_ERR]', err);
      setLoading(false);
    });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
          ...
          Alertes & Courses Attribuées
        </h1>
        <p className="text-xs sm:text-sm text-slate-500">
          Notifications en direct de nouvelles courses à livrer.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-8 text-center border border-slate-200/80 shadow-sm space-y-2">
          ...
          <p className="text-xs text-slate-400">Aucune alerte en attente.</p>
        </div>
      ) : (
