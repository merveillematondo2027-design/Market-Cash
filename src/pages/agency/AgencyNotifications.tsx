...
export default function AgencyNotifications() {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where('userId', 'in', [user.uid, 'chef_agence', 'all'])
    );
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      console.error('[AGENCY_NOTIFICATIONS_ERR]', err);
      setLoading(false);
    });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
          ...
          Alertes & Notifications Agence
        </h1>
        <p className="text-xs sm:text-sm text-slate-500">
          Toutes les alertes d'achats de cartes, livraisons et statut du réseau.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-8 text-center border border-slate-200/80 shadow-sm space-y-2">
          ...
          <p className="text-xs text-slate-400">Vous n'avez aucune notification en attente.</p>
        </div>
      ) : (
