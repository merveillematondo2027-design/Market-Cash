...
export default function BusinessLayout(){
  ...
  const nav=[
    ...
    {name:'Developer',path:'/business/developer',icon:Code2},
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-800">
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-white px-4"><Link to="/business/home" className="font-black text-blue-950">MARKET-<span className="text-amber-500">CASH</span> BUSINESS</Link><button onClick={()=>setLogout(true)} className="rounded-xl p-2 text-red-600" aria-label="Se déconnecter"><LogOut size={20}/></button></header>
    <main className="pb-20"><Outlet/></main>
    <nav className="fixed inset-x-0 bottom-0 z-50 grid h-16 grid-cols-5 border-t bg-white">{nav.map(item=>{const Icon=item.icon;const active=location.pathname.startsWith(item.path);return <Link key={item.name} to={item.path} className={cn('flex flex-col items-center justify-center gap-1 text-[9px] font-black',active?'text-blue-950':'text-slate-400')}><Icon size={19}/><span>{item.name}</span></Link>})}</nav>
