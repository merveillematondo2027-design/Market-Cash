...
export default function BusinessLayout(){
  ...
  const mode=user?.businessAccountType||'merchant';const developer=mode==='direct_developer'||mode==='api_provider';
  const nav=developer?[{name:mode==='api_provider'?'Partner':'Console',path:'/business/developer',icon:Code2},{name:'Profil',path:'/business/profile',icon:User}]:[
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-800">
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-white px-4"><Link to={home} className="font-black text-blue-950">MARKET-<span className="text-amber-500">CASH</span> {developer?(mode==='api_provider'?'PARTNER':'DEVELOPER'):'BUSINESS'}</Link><button onClick={()=>setLogout(true)} className="rounded-xl p-2 text-red-600" aria-label="Se déconnecter"><LogOut size={20}/></button></header>
