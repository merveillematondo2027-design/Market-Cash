...
export default function KycGate({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{let active=true;(async()=>{if(!user?.uid){if(active)setStatus('required');return}const snap=await getDoc(doc(db,'kyc_requests',user.uid));if(active)setStatus(snap.exists()&&snap.data()?.status==='approved'?'approved':'required')})().catch(()=>active&&setStatus('required'));return()=>{active=false}},[user?.uid]);
