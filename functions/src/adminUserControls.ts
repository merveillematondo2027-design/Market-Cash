...
export const adminGetUserControl = onCall({region:REGION}, async(request) => {
  ...
  for (const currency of CURRENCIES) {
    ...
    wallets[currency] = data ? {
      ...
      marketCashId: data.marketCashId || '',
  ...
  return {
    ...
    securityResetAt: Number(user.securityResetAt || 0),
    wallets,
  };
});
...
export const adminUpdateUserControl = onCall({region:REGION}, async(request) => {
  ...
  const target = await requireTarget(targetUid);
  const targetData = target.data() || {};
  if (targetUid === actorId && ['set_account_status','reset_pin'].includes(action)) {
    throw new HttpsError('failed-precondition', 'Cette action de sécurité ne peut pas être appliquée à votre propre compte depuis cette page.');
  }
  ...
  if (action === 'set_account_status') {
    ...
    if (status !== 'active') {
      ...
      for (const currency of CURRENCIES) {
        ...
        if ((await ref.get()).exists) batch.update(ref,{status:'frozen',updatedAt:now});
      }
      await batch.commit();
    }
    await audit(actorId,targetUid,'ADMIN_ACCOUNT_STATUS_CHANGED',{status,previous:targetData.accountStatus || 'active'});
    return {ok:true};
  }
  ...
  if (action === 'set_wallet_status') {
    ...
    const selected = currency === 'ALL' ? [...CURRENCIES] : [currency as Currency];
    const batch = db.batch();
    let changed = 0;
    for (const c of selected) {
      const ref = db.doc(`wallet_accounts/${walletId(targetUid,c)}`);
      if ((await ref.get()).exists) { batch.update(ref,{status,updatedAt:now}); changed++; }
    }
    if (changed) await batch.commit();
    await audit(actorId,targetUid,'ADMIN_WALLET_STATUS_CHANGED',{status,currency,changed});
    return {ok:true,changed};
  }

  if (action === 'reset_pin') {
    await target.ref.update({pinHash:FieldValue.delete(),useBiometrics:false,securityResetAt:now,securityResetBy:actorId,updatedAt:now});
    await audit(actorId,targetUid,'ADMIN_PIN_RESET');
    await db.collection('notifications').add({userId:targetUid,title:'Sécurité du compte',message:'Votre code PIN Market-Cash a été réinitialisé par l’administration. Définissez un nouveau PIN lors de votre prochaine utilisation.',type:'info',category:'security',read:false,createdAt:now});
    return {ok:true};
  }

  if (action === 'disable_biometrics') {
    await target.ref.update({useBiometrics:false,updatedAt:now});
    await audit(actorId,targetUid,'ADMIN_BIOMETRICS_DISABLED');
    return {ok:true};
  }

  if (action === 'set_note') {
    const note = String(request.data?.note || '').trim().slice(0,2000);
    await target.ref.update({adminNote:note,adminNoteUpdatedAt:now,adminNoteUpdatedBy:actorId,updatedAt:now});
    await audit(actorId,targetUid,'ADMIN_NOTE_UPDATED',{length:note.length});
    return {ok:true};
  }
