...
function normalizeAppPin(value: any) {
  ...
  if (!/^\d{4,10}$/.test(pin)) throw new HttpsError('invalid-argument', 'Le code secret de l’application doit contenir 4 à 10 chiffres.');
  return pin;
}
...
export const walletToLocalCardWithCvv = onCall({ region: REGION }, async request => {
  ...
  return db.runTransaction(async tx => {
    ...
    tx.set(txRef, { id: txId, reference, type: 'wallet_to_local_card', status: 'settled', currency, amount, feeAmount:0, totalDebited:amount, userId: uid, userIds: [uid], cardId: card.cardId, sourceWalletId: walletRef.id, destinationCardWalletId: cardBalanceRef.id, rail: 'market_cash_local_card', authenticatedBy: 'local_cvv', createdAt: now, updatedAt: now });
...
export const marketCashTransferWithCvv = onCall({ region: REGION }, async request => {
  ...
  return db.runTransaction(async tx => {
    ...
    const feeAmount = await transactionFee('market_cash_transfer', currency, amount);
...
export const merchantPaymentFromLocalCardWithCvv = onCall({ region: REGION }, async request => {
  ...
  return db.runTransaction(async tx => {
    ...
    const feeAmount=await transactionFee('merchant_payment',currency,amount);
    const totalDebited=roundTransactionMoney(amount+feeAmount);
...
export const agentLocalCardCashOutWithCvv = onCall({ region: REGION }, async request => {
  ...
  return db.runTransaction(async tx => {
    ...
    const feeAmount=await transactionFee('agent_cash_out',currency,amount);
    const totalDebited=roundTransactionMoney(amount+feeAmount);
    if (Number(cardBalance.availableBalance || 0) < totalDebited) throw new HttpsError('failed-precondition', 'Solde de la carte locale insuffisant, frais compris.');
    const now = Date.now();
