...
export const merchantPaymentFromLocalCardWithCvvV2=onCall({region:REGION},async request=>{
  ...
  return db.runTransaction(async tx=>{
    ...
    tx.set(db.collection('ledger_entries').doc(),{transactionId:txId,cardWalletId:source.id,userId:payerUid,direction:'debit',amount:totalDebited,currency,entryType:'merchant_payment_total_debit',createdAt:now});tx.set(db.collection('ledger_entries').doc(),{transactionId:txId,walletId:destination.id,userId:merchant.uid,direction:'credit',amount,currency,entryType:'merchant_payment_received',createdAt:now});tx.set(db.collection('ledger_entries').doc(),{transactionId:txId,revenueWalletId:revenueRef.id,direction:'credit',amount:feeAmount,currency,entryType:'merchant_payment_fee',createdAt:now});tx.set(db.collection('notifications').doc(),{userId:merchant.uid,title:'Paiement reçu',message:`Vous avez reçu ${amount} ${currency} par carte locale Market-Cash.`,type:'success',category:'general',read:false,transactionId:txId,createdAt:now});
