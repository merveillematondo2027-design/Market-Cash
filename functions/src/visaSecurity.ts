...
export const revealVisaCardSecureData=onCall({region:REGION},async request=>{
  ...
  return{cardId,tier:tierOf(d),cardNumber:String(d.cardNumber||''),cardHolder:String(d.cardHolder||d.cardHolderName||d.userName||''),expiryStart:String(d.expiryStart||d.validFrom||''),expiryEnd:String(d.expiryEnd||d.expiry||d.validUntil||''),cvv:String(d.cvv||''),status:String(d.status||'active')};
});
