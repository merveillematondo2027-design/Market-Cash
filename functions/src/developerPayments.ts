...
export const marketCashApiCardPayment = onRequest({ region: REGION }, async (req,res) => {
  ...
  try{
    ...
    res.status(200).json({status:'approved',approved:true,duplicate:Boolean((result as any).duplicate),reference:(result as any).reference,externalReference,amount,feeAmount:(result as any).feeAmount||0,netAmount:(result as any).netAmount??amount,totalDebited:(result as any).totalDebited??amount,apiUsageFeeUsd:partner?pricing.wholesaleUsd:0,directFeePercent:partner?0:pricing.directPercent,currency,pricingTier:partner?'wholesale':'direct',developer:auth.developer.companyName,app:auth.app.appName});
